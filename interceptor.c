...
{
    spin_lock(&pidlist_lock); // because you access the pid_list
    del_pid(current->pid);
...
asmlinkage long interceptor(struct pt_regs reg) {
    ...
    } else if (table[reg.ax].monitored == 2 && table[reg.ax].listcount == 0){
        log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
    } else if (table[reg.ax].monitored == 2 && check_pid_monitored(reg.ax, current->pid) == 0){
        log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
    }
    spin_unlock(&pidlist_lock);
    ...
    if (cmd == REQUEST_SYSCALL_INTERCEPT || cmd == REQUEST_SYSCALL_RELEASE){
        ...
        if (current_uid() != 0){
            ...
            if (table[syscall].intercepted != 1){
                ...
                sys_call_table[syscall] = &interceptor; // because it takes in a function pointer
        ...
        if (pid < 0 || (pid != 0 && pid_task(find_vpid(pid), PIDTYPE_PID) == 0)){
            return -EINVAL;
        }
        if (current_uid() != 0){
            if (pid == 0 || check_pid_from_list(current->pid, pid) != 0){ // check for pid == 0 first
                return -EPERM;
            }
        }
        if (cmd == REQUEST_START_MONITORING) {
            spin_lock(&pidlist_lock);
            if (check_pid_monitored(syscall, (pid_t) pid) != 0 && pid != 0) {
                spin_unlock(&pidlist_lock);
                return -EBUSY;
            }
            if (check_pid_monitored(syscall, pid) == 1 && table[syscall].monitored != 2){ // pid is already monitored
                spin_unlock(&pidlist_lock);
                return -EBUSY;
            }
            if (pid == 0 && table[syscall].monitored == 2 && table[syscall].listcount == 0){ // every pid is monitored already
                spin_unlock(&pidlist_lock);
                return -EBUSY;
            }
            if (check_pid_monitored(syscall, pid) == 0 && table[syscall].monitored == 2){ // every pid is monitored
                spin_unlock(&pidlist_lock);
                return -EBUSY;
            }
            if (table[syscall].monitored != 2) {
                ...
                if (del_pid_sysc(pid, syscall) != 0){ // deleting because it is a blacklist
                    spin_unlock(&pidlist_lock);
                    return -EINVAL;
                }
            }
            ...
            spin_lock(&pidlist_lock);
            if ((check_pid_monitored(syscall, (pid_t) pid) == 0 && pid != 0) || table[syscall].intercepted == 0){
                spin_unlock(&pidlist_lock);
                return -EINVAL;
            }
            if (pid == 0 && table[syscall].monitored == 0){ // if it's not monitoring any pid
                spin_unlock(&pidlist_lock);
                return -EINVAL;
            }
            if (pid == 0 && table[syscall].listcount > 0){
                destroy_list(syscall);
            } else if (pid==0 && table[syscall].listcount == 0){
                table[syscall].monitored = 0;
            } else {
                if (table[syscall].monitored == 2){
                    add_pid_sysc(pid, syscall); // adding because it is a blacklist
                }
                else if (del_pid_sysc((pid_t) pid, syscall) != 0 ){
                    spin_unlock(&pidlist_lock);
                    return -EINVAL;
                }
            }
...
static void exit_function(void)
{
    ...
    for (; i < NR_syscalls; i++){
        if (table[i].intercepted == 1) {
            spin_unlock(&pidlist_lock); // because it locks again in the function
            my_syscall(REQUEST_SYSCALL_RELEASE, i, 0);
            spin_lock(&pidlist_lock); // because we need to lock it to access the if condition
        }
    }
    spin_unlock(&pidlist_lock);
    spin_lock(&calltable_lock);
